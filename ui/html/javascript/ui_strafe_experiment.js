...
function refresh_strafe_hud(intent_mask, obj, yaw, speed_x, speed_y, speed_z){
    ...
    if (strafe_state_check(intent_mask, yaw, speed_x, speed_y, speed_z)) {
        ...
        var lineHeight = strEff<0 ? '100%' : Math.min( 100, Math.abs(2*wallHeight/Math.cos(keydir_rad+veldir_rad) ) ) + '%';
        var t_spin_dir = Math.sin(keydir_rad+veldir_rad) > 0 ? true : false;
        ...
        if (strafe_data.spin!=t_spin_dir) strafe_data.spin = t_spin_dir;
        if (strafe_data.lineheight!=lineHeight) strafe_data.lineheight = lineHeight;
        if (update_efficiency) {
	        let dot_trans_str = -Math.abs(strEff/2)+"%";
	        let eff_color_rgb = 'rgb(255,255,255)';
	        let throttle_str  = '0%';
	        if (speed_hor>base_speed) {
	            if (strEff<0) {
	              //eff_color_rgb='rgb(200,50,50)';
	              eff_color_rgb='rgb(255,0,0)';
	              throttle_str = 75 - 25*unsmoothed_efficiency +'%';
	              ...
	              throttle_str = 50*unsmoothed_efficiency +'%';
	            }
	        }
	        if (strafe_data.dot_trans!=dot_trans_str) strafe_data.dot_trans = dot_trans_str;
	        if (strafe_data.throttle!=throttle_str) strafe_data.throttle = throttle_str;
	        if (strafe_data.color!=eff_color_rgb) strafe_data.color = eff_color_rgb;
	        if (strafe_data.efficiency!=strEff) strafe_data.efficiency = strEff;
//	        if (strafe_data.horsepower!=strPwr) strafe_data.horsepower = strPwr;
//	        if (strafe_data.torque!=strAcc) strafe_data.torque = strAcc;
	    }
    }
}
