        ...
        if (type == "j") {
            ...
            switch(action) {
                ...
                case "competitive-season":
                    ...
                    global_leaderboards_season = global_competitive_season.comp_season_id;
                    break;
                case "chat-msg":
...
function getCompSeasonModes(comp_season_id, cb) {
    ...
    let comp_key = null;
    for (let key in global_competitive_data) {
        if (global_competitive_data[key].id == comp_season_id) {
            comp_key = key;
            break;
        }
    }

    // id doesn't exist in our hardcoded list of seasons
    if (comp_key === null) {
        cb([])
        return;
    }

    // we already have a list of modes stored for this season
    if (global_competitive_data[comp_key].hasOwnProperty("modes")) {
        cb(global_competitive_data[comp_key].modes);
        return;
    }

    // get the list of modes from the api
    api_request("GET", "stats/seasonmodes", { "comp_season_id": comp_season_id}, function(modes) {
        for (let m of modes) {
            m.name = getModeName(m.mode_key, m.mode_name, m.instagib);
            m.vs = getVSType(m.team_count, m.team_size);
        }

        global_competitive_data[comp_key].modes = modes;
        cb(modes);
    });
