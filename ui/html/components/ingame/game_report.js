...
function selectPlayer(name, stats, show, game_mode) {
    ...
    if (GLOBAL_ABBR.STATS_KEY_WEAPONS in stats) {
        for (let s of stats[GLOBAL_ABBR.STATS_KEY_WEAPONS]) {
            ...
            if (!global_weapons_in_scoreboard.includes(s[GLOBAL_ABBR.STATS_KEY_ITEM_IDX])) continue;

            let weapon = _createElement("div", "weapon");

            let w_data = undefined;
            if (s[GLOBAL_ABBR.STATS_KEY_ITEM_IDX] in global_weapon_idx_name_map2) {
                w_data = global_item_name_map[global_weapon_idx_name_map2[s[GLOBAL_ABBR.STATS_KEY_ITEM_IDX]]];
            }
    ...
    if (GLOBAL_ABBR.STATS_KEY_ITEMS in stats) {
        let pickups_cont = _createElement("div", "pickups");

        let item_lookup_map = {};
        for (let pickups of stats[GLOBAL_ABBR.STATS_KEY_ITEMS]) item_lookup_map[pickups[GLOBAL_ABBR.STATS_KEY_ITEM_IDX]] = pickups[GLOBAL_ABBR.STATS_KEY_COUNT];

        for (let item of global_item_pickups_in_scoreboard) {
            if (!item_lookup_map.hasOwnProperty(item)) continue;
            
            let item_cont = _createElement("div", "item_cont");
            let icon = _createElement("div", "icon");
            if (global_item_name_map.hasOwnProperty(item)) icon.style.backgroundImage = "url("+global_item_name_map[item][2]+"?fill="+global_item_name_map[item][0]+")";
            item_cont.appendChild(icon);
            item_cont.appendChild(_createElement("div", "count", item_lookup_map[item]));

            pickups_cont.appendChild(item_cont);
        }
