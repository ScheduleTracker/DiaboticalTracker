...
global_onload_callbacks_other.push(function() {
    ...
    let rarity_ids = Object.keys(global_rarity_map);
    let rarity_filter = _id("customize_dropdown_rarity_filter");
    for (let rid of rarity_ids) {
        let opt = _createElement("div", "i18n");
        opt.dataset.i18n = global_rarity_map[rid].i18n;
        opt.dataset.value = rid;
        opt.innerHTML = localize(global_rarity_map[rid].i18n);
        rarity_filter.appendChild(opt);
    }
    ui_setup_select(rarity_filter);

    global_input_debouncers['customize_input_text_filter'] = new InputDebouncer(function(){ onInputFilter(); });
    function onInputFilter() {
        window.currentItemsPage = 0;
        updateDecalsList(); 
    }

    bind_event('set_weapon_skin_options', function (weaponsJSON) {
         var weapons = JSON.parse(weaponsJSON); 
    });

    window.decals_per_page = 18;

    window.currentItemsPage = 0;

    window.customize_filter_mode = "";
    window.customize_order_mode = "";
    window.decals = []; 
    window.decals_map = {};
    window.songs = [];
    window.songs_map = {};
    ...
    bind_event('set_asset_browser_content', function (decalsJSON) {

        var decals = JSON.parse(decalsJSON);  
        //console.log(_dump(decals));
        
        for(var i = 0; i < decals.length; i++){
            decals[i].cleanId = decals[i].id.replace(/_/g, " ");
            window.decals_map[decals[i].id] = decals[i];
        }
    ...
    bind_event('set_song_list', function (song_list_str) {

        var song_data = JSON.parse(song_list_str);

        var songs = [];
        for(i = 0; i < song_data.length; i++)
        {
            var song = {};
            song.cleanId = song_data[i]["friendly_name"];
            song.id = song_data[i]["name"];
            song.quality = 2;
    ...
    _id("customization_items_next_page").addEventListener("click", function()
    {
        if((window.currentItemsPage ) * window.decals_per_page + window.decals_per_page < window.itemsToShow.length)
        {
            window.currentItemsPage += 1;
            updateDecalsList();
        }
    });


    _id("customization_items_prev_page").addEventListener("click", function()
    {
        if(window.currentItemsPage > 0)
        {
            window.currentItemsPage -= 1;
            updateDecalsList();
        }
    ...
    addListener(_id("customize_dropdown_rarity_filter"), function(opt, field) {
        window.customize_filter_mode = opt.dataset.value;
        window.currentItemsPage = 0;
        updateDecalsList();
    });

    addListener(_id("customize_dropdown_show_order"), function(opt, field)
    {
        window.customize_order_mode = opt.dataset.value;
        updateDecalsList();
    });
...
function showPreview(id) {
    if (window.decals_map[id]) {
        let template = window.jsrender.templates(_id("customization_decal_detail").textContent);
        let htmlOutput = template.render(window.decals_map[id]);
        _html(_id("customization_preview"), htmlOutput);
    }
}

function char_shell(element) {
    //engine.call("echo", "shell=" + $(element).data("shellid"));
    engine.call("set_string_variable", "game_skin", "" + element.dataset.shellid);
    set_tool("add_shell", element);
}
...
function updateDecalsList() {
    ...
    var start = window.currentItemsPage * window.decals_per_page;
    var end = start + window.decals_per_page;

    var listItems = [];

    var items = [];

    if(window.currentCustomizePage == "songs") {
        listItems = window.songs;
    } else {
        listItems = window.decals;
    }

    if(window.customize_filter_mode == "all") {
        items = listItems;
    } else {
        items = listItems.filter(decal => decal.quality == window.customize_filter_mode);
    }

    var textFilter = _id("customize_input_text_filter").value;

   
    const result2 =  items.filter(decal => decal.cleanId.toLowerCase().includes(textFilter.toLowerCase()));
    window.itemsToShow = result2;

    if( window.customize_order_mode === "name"){
        window.itemsToShow.sort(function (a, b) {
            if (a.cleanId < b.cleanId) return -1;
            else if (a.cleanId > b.cleanId) return 1;
            return 0;
          });
    } else {
        window.itemsToShow.sort((a, b) => b.quality - a.quality);
    }


    var template = window.jsrender.templates(_id("customization_decal_entrys").textContent);
    var htmlOutput = template.render({'selected': global_customize_selected_decal, 'items': window.itemsToShow.slice(start, end)});
    
    _html(_id("customization_items_list"), htmlOutput);

    _html(_id("customization_items_page_count"), (window.currentItemsPage + 1) + "/" + (Math.ceil( window.itemsToShow.length / decals_per_page)));

}

function open_customization_section(button,type) {
    _for_each_with_selector_in_parent(_id("customize_screen"), ".customization_menu .active", function(el) {
        el.classList.remove("active");
    });
    button.classList.add("active");

    _empty(_id("customization_preview"));

    if (type == "decals") {
        anim_hide(_id("skin_screen_songs"));
        anim_show(_id("skin_screen_decals"), window.fade_time);
    
        _id("customization_items_list").style.display = "flex";
    
        engine.call("on_show_customization_screen_eggbot", true);
    
        window.currentCustomizePage = "decals";
        window.currentItemsPage = 0;
    
        updateDecalsList();
        showPreview(global_customize_selected_decal);
    }
    if (type == "songs") {
        anim_show(_id("skin_screen_songs"), window.fade_time);
        anim_hide(_id("skin_screen_decals"), window.fade_time);
        
        window.currentCustomizePage = "songs";
        window.currentItemsPage = 0;    
    ...
    if (type == "weapons") {
        anim_hide(_id("skin_screen_decals"), window.fade_time);
        anim_hide(_id("skin_screen_songs"), window.fade_time);

        engine.call("on_show_customization_screen_weapon", true);

        window.currentCustomizePage = "weapons";
        window.currentItemsPage = 0;
